 ...
 */
avl_t *avl_insert_recursive(avl_t **tree, avl_t *parent,
		avl_t **new_root, int value)
{
	int bfactor;

	if (*tree == NULL)
		return (*new_root = binary_tree_node(parent, value));

	if ((*tree)->n > value)
	{
		(*tree)->left = avl_insert_recursive(&(*tree)->left, *tree, new_root, value);
		if ((*tree)->left == NULL)
			return (NULL);
	}
	else if ((*tree)->n < value)
	{
		(*tree)->right = avl_insert_recursive(&(*tree)->right, *tree, new_root, value);
		if ((*tree)->right == NULL)
			return (NULL);
	}
	else
